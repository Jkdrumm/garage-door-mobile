...
export function useUpdateDeviceName(
  options?: UseMutationOptions<void, {deviceName: string}>,
) {
  const queryClient = useQueryClient();
  return useMutation<void, {deviceName: string}>(
    [UPDATE_DEVICE_NAME_QUERY_KEY],
    {
      ...options,
      onSuccess: (data, variables, context) => {
        // TODO: Update the data in AsyncStorage
        queryClient.setQueryData<string>(
          [DEVICE_NAME_QUERY_KEY],
